...
public class Program
{
    public static void Main(string[] args)
    {
        ...
        // Storage
        if (string.Equals(storage.Mode, "Sql", StringComparison.OrdinalIgnoreCase))
        {
            builder.Services.AddDbContext<AppDbContext>(opt =>
            {
                if (string.Equals(storage.SqlProvider, "SqlServer", StringComparison.OrdinalIgnoreCase))
            ...
            if (!string.IsNullOrWhiteSpace(storage.Redis))
            {
                builder.Services.AddSingleton<IConnectionMultiplexer>(_ => ConnectionMultiplexer.Connect(storage.Redis));
        ...
        {
            builder.Services.AddSingleton<IConnectionMultiplexer>(_ => ConnectionMultiplexer.Connect(storage.Redis));
            builder.Services.AddScoped<IReceiptRepository>(sp => new RedisReceiptRepository(sp.GetRequiredService<IConnectionMultiplexer>(), storage.ReceiptPrefix));
            builder.Services.AddScoped<IShortLinkRepository>(sp => new RedisShortLinkRepository(sp.GetRequiredService<IConnectionMultiplexer>(), storage.ShortPrefix));
            builder.Services.AddSingleton<IOtpStore, RedisOtpStore>();
            builder.Services.AddSingleton<IRateLimitService, RedisRateLimitService>();
        }
        ...
        app.MapPost("/api/shorten", async (ShortenRequest req, IShortLinkRepository repo, IRateLimitService rl) =>
        {
            ...
            var s = new Data.ShortLinkEntity
            {
                Code = code,
                LongUrl = req.LongUrl,
                Usage = 0,
        ...
        app.MapPost("/tcrm/issue", async (IssueRequest req, IReceiptRepository recRepo, IShortLinkRepository shortRepo, JwtService jwtService) =>
        {
            ...
            var rec = new Data.ReceiptEntity
            {
                ...
                ItemsJson = JsonSerializer.Serialize(req.Items ?? new List<Models.ReceiptItem>()),
                ExpiresAt = expiresAt,
        ...
        app.MapGet("/api/receipt/{id}", async (string id, IReceiptRepository repo) =>
        {
            ...
            var items = System.Text.Json.JsonSerializer.Deserialize<List<Models.ReceiptItem>>(rec.ItemsJson) ?? new();
